...
```{r}
# We can use the `fix(x)` function to make adjustments to the data like we would in an excel file, though the operation is cpu costily 
## fix(Boston)
names(Boston)
# We can also use `?Boston` to get more information about the dataset
```
...
```{r}
# Fit the median value based on economic status from the Boston dataset
lm.fit = lm(medv ~ lstat, data = Boston)

# Use the `attach()` function to attache a database to the R search path
attach(Boston)

# Since we attached the Boston dataset, we no longer need to define it within the function
...
```{r}
# Just calling the `lm.fit` variable returns the coefficients
lm.fit

# If we want more info, use `summary()`
summary(lm.fit)
```
...
```{r}
# Get the names of the outputs in the `lm.fit` function
names(lm.fit)
...
```{r}
# Predict and provide confidence interval
...
Here Model 1 represents the linear submodel containing only one predictor, `lstat`, while Model 2 corresponds to the larger quadratic model that has two 2
predictors, `lstat` and `lstat`$^2$ . The `anova()` function performs a hypothesis test comparing the two models. The null hypothesis is that the two models fit the data equally well, and the alternative hypothesis is that the full model is superior. Here the $F$-statistic is 135 and the associated $p$-value is virtually zero. This provides very clear evidence that the model containing the predictors `lstat` and `lstat`$^2$ is far superior to the model that only contains the predictor `lstat`. This is not surprising, since earlier we saw evidence for non-linearity in the relationship between medv and lstat. If we type:
